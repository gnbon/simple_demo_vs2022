...
int main(int argc, char *argv[]) {
  ...
  swprintf_s(output_directory, MAX_FILE_PATH,L"%lsannotation_summary/", output_path);
  _wmkdir(output_directory);

  {
    // Load a document
    FS_PDFDOC_HANDLE doc = NULL;
    FS_ANNOTATIONSUMMARY_HANDLE commentssummary = NULL;
    FS_ANNOTATIONSUMMARYSETTINGS_HANDLE summarysettings;
    wchar_t summary_savepath[MAX_FILE_PATH] = { 0 };
    ...
    FSDK_AnnotationSummarySettings_SetAnnotType(summarysettings, e_FSAnnotTypeUnknownType, TRUE);
    FSDK_AnnotationSummarySettings_SetFileTitle(summarysettings,L"AnnotationSummaryFileTitle");
    FSDK_AnnotationSummarySettings_EnableOutputNoAnnotationPage(summarysettings, TRUE);
    ...
    swprintf_s(summary_savepath, MAX_FILE_PATH, L"%lsAnnotationSummary_generate.pdf", output_directory);
    error_code = FSDK_AnnotationSummary_StartSummarize(commentssummary, summary_savepath, summarysettings, NULL, &return_value);
    if (error_code != e_FSErrSuccess) {
