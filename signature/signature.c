...
void AdobePPKLiteSignature(FS_PDFDOC_HANDLE pdf_doc, FS_BOOL use_default) {
  ...
  if (!use_default) {
    new_signature = AddSiganture(pdf_page, gsignature_callback_data.sub_filter_.str);
  ...
  swprintf_s(signed_pdf_path, MAX_FILE_PATH, L"%ssigned_newsignature.pdf", output_directory);

  if (use_default)
    swprintf_s(signed_pdf_path, MAX_FILE_PATH, L"%ssigned_newsignature_default_handler.pdf", output_directory);  
...
int main(int argc, char *argv[]) {
  ...
  int i;
  swprintf_s(output_directory, MAX_FILE_PATH, L"%ssignature/", output_path);
  ...
  for (i = 0; i < 2; i++) {
    ...
    FS_PROGRESSIVE_HANDLE progressive = NULL;
    FSErrorCode code = FSDK_PDFDoc_Create0(input_file, &pdf_doc);
